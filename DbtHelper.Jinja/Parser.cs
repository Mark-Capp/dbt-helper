...
namespace DbtHelper.Jinja
{
    internal class Parser
    {
        ...
        {
            return _lex.GetToken(i) switch
            {
                { IsInvalid: true } => null,
                { TokenType: TokenType.StatementBegin } t => ParseStatementClause(t),
                { TokenType: TokenType.ExpressionBegin } t => ParseExpressionClause(t),
                _ => ParseTextClause(i)
            };
        }
                ...
                case TokenType.For:
                    {
                        ...
                        if (loopVariable is not SymbolExpression)
        ...
        #region Expression
        private Expression ParseExpression(int i) => ParseFilterExpression(i);
        ...
        private Block ParseIfBlock(IList<Clause> clauseList, ref int i)
        {
            ...
            if (i >= clauseList.Count || clauseList[i] is not EndIfClause)
        ...
        {
            (var row, var column) = TextUtils.GetCharPosition(_s, charIndex);
